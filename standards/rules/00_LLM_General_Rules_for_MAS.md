...
- For historical reference, see `docs/_RULES_MAS_System_Rules/ARCHIVE/1.3_System_DNU.md` (archived/legacy)
...
## Ancestral & Indigenous Knowledge Framing

When describing ancestral, Indigenous, or traditional ecological knowledge in user-facing text, solution descriptions, evidence entries, documentation, or expert/local-knowledge data sources, use the **"Time-Tested, Proven"** framing. Treat these methods as validated infrastructure with a multi-century track record, not as folklore, backstory, or cultural color.

### **Do**
- Present the method as proven engineering that modern practice is catching up to.
- Name the specific peoples and places when the historical or ethnographic record supports it (e.g., Nuwuvi, Ancestral Puebloans).
- Cite peer-reviewed, tribal, or federal agency sources — not secondary popularizations.
- When used inside `nature_based_solutions.json`, encode as `claim_type: mechanism`, `study_type: institutional_report` or `expert_opinion`, and register the cited sources in the top-level `citations` registry with the appropriate `evidence_tier`.

### **Do Not**
- Use words like "primitive," "pre-modern," "pre-scientific," or "folklore."
- Imply Indigenous methods are inferior precursors to modern engineering — the point is that they are frequently *equivalent or superior* and simply older.
- Strip attribution; the people and place stay in the sentence.
- Generalize one community's practice to "Indigenous knowledge" broadly — be specific.

### **Anchor Example** (use this phrasing pattern in product copy, solution descriptions, and evidence notes)

> Modern civil engineers essentially "rediscovered" that the most efficient way to clean water in the desert is not a high-tech plant, but the exact wetland-slowing method used by the Nuwuvi and Puebloans for millennia. Incorporating time-tested infrastructure that is proven, efficient, and effective is just smart engineering.

### **Reference Sources for This Framing**

- Springs Preserve — Mission and History (Nuwuvi in Las Vegas): https://www.springspreserve.org/about/mission-history.html
- National Park Service — Ancestral Puebloan Water Conservation (teacher background): https://www.nps.gov/common/uploads/teachers/lessonplans/Appendix-1-Ancestral-Puebloan-Background-Information.pdf
- EPA / Nevada DEP — Restoring Water Quality in Las Vegas Wash: https://www.epa.gov/sites/default/files/2015-12/documents/nv_lvwash.pdf

### **Cross-References**

- Evidence schema and `citations` registry conventions: [src/multi_agent_system/data/nature_based_solutions.json](../../src/multi_agent_system/data/nature_based_solutions.json) — see `_metadata.schema` and `_metadata.unit_policy`.
- Local/Indigenous expert data source catalog: [src/multi_agent_system/data/local_expert_knowledge_data_sources_examples.json](../../src/multi_agent_system/data/local_expert_knowledge_data_sources_examples.json).
...
### **April 14, 2026**
- **Added**: "Ancestral & Indigenous Knowledge Framing" section establishing the "Time-Tested, Proven" framing rule, anchor example, and reference sources (Springs Preserve, NPS, EPA Las Vegas Wash). Mirrored in `00_cursor_rules.md`.
...
### **December 12, 2025**
- **File Renamed**: Changed from `00_Cursor_rules_for_MAS_Work.md` to `00_LLM_General_Rules_for_MAS.md`
...
- **Updated Enforcement Guidelines**: Changed primary reference from 1.3_System_DNU.md to this file (00_LLM_General_Rules_for_MAS.md)
...
- **Cross-Reference Consistency**: Standardized all references to use full path `docs/_RULES_MAS_System_Rules/ARCHIVE/1.3_System_DNU.md`
