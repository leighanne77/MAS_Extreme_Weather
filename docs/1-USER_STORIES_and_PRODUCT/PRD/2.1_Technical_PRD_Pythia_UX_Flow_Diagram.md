...
5. [2.1_Technical_PRD.md - restating the Considerations for UX](#21_technical_prdmd---restating-the-considerations-for-ux)
...
9. [Notifications requirements](#notifications-requirements)
...
## **2.1_Technical_PRD.md - restating the Considerations for UX**
...
→ "Yes" → "Where are you looking to invest?" → "Alabama shipbuilding"
...
7. **Confidential Notifications:** All monitoring stays within Tool 
...
#### **Filter System Alignment with System Constraints**
- **Filter Approach Refinement**: Updated all user-specific filter sections to align with system constraint that Pythia cannot access internal financial data (IRR calculations, portfolio data, loan data, etc.)
- **Framework-Based Filters**: All filter sections now emphasize providing frameworks, data, and benchmarks rather than direct results
- **Private Equity Investor Filters**: Updated to focus on "Construction Cost Risk Factors," "Timeline Impact Analysis," and "ROI Data" instead of "Highest IRR"
- **Rationale Added**: Each user type section now includes rationale explaining why filters provide frameworks rather than direct metrics
- **Notification System**: Enhanced notification requirements section to emphasize optional monitoring and confidentiality

#### **Documentation Consistency**
- **Terminology Alignment**: Aligned filter descriptions with new terms added to `0_Terms_used.md` (Bioregion, Exit Value, Resilience, Multisolving Risk Modeling, Resilience Add Ons, NBS)
- **Cross-Reference Updates**: Updated references to reflect renamed directory structure (`0_PROJECT_STRUCTURE_and_TERMS_Used` instead of `a_PROJECT_STRUCTURE_and_TERMS_Used`)

#### **Content Enhancements**
- **User Story Clarification**: Enhanced user story examples to clarify that users want data to calculate their own metrics, not direct results
- **Decision Support Focus**: Emphasized throughout that Tool provides decision support, not decision-making automation
- **Confidentiality Emphasis**: Strengthened language around data confidentiality and user control in notification system
