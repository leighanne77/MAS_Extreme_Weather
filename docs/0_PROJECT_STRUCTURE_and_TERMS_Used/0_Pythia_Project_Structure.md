...
**Date Last Updated**: December 19, 2025
...
├── tests/                         # Test files
├── dependencies/                  # External dependencies (cloned repositories)
│   ├── erddap2mcp/               # ERDDAP MCP Server repository
│   └── datagov-mcp-server/       # Data.gov MCP Server repository
...
### Python Data Loaders (non-API, non-MCP)
- **pollinator_data.py**: Pollinator data scraper/loader (quarterly refresh)
- **usda_climate_hubs_data.py**: USDA Climate Hubs data loader (quarterly refresh)
- **fhfa_pudb_loader.py**: FHFA Public Use Database loader for property/loan/home price data [Ref]
- **nbi_loader.py**: National Bridge Inventory loader (CSV/ASCII)
- **data_loader.py**: General-purpose local data loader
- **ers_ers.py**: USDA Economic Research Service data loader (local, not an API client)
- **nass_crop_reports.py**: NASS crop reports loader (local, not an API client)
- **cache_utils.py**: Local data caching utilities
- **enhanced_data_sources.py**: Enhanced data source manager (local, not an API client)
- **nature_based_solutions_source.py**: Nature-based solutions data loader (if not MCP-integrated)

### Static/Reference Data (JSON)
- **nature_based_solutions.json**
- **coastal_areas_infrastructure_needs.json**
- **local_expert_knowledge_data_sources_examples.json**
- **economic_impact_data.json**
- **funding_sources_NSB.json**
- **erddap_servers.json**
- **historical_weather_events.json**
- **regional_opportunities.json**
- **regional_risk_profiles.json**
- **Biodiversity_Credit_Revenue_Streams.json**
- **government_data_sources_examples.json**
...
### **December 19, 2025**
- Added new API integrations: BLS, FHFA HPI, EIA, OSM, OpenFEMA, USDA NASS, USDA Snow AWDB, Census, EPA WQX, EPA STORET, National Bridge Inventory, FHFA PUDB loader (as [Ref]), pollinator and USDA Climate Hubs data loaders (quarterly refresh).
- Updated Data Layer documentation to reflect new modules and [Ref] status for non-API sources.
- Added TODO for Google Earth Engine integration (see TODO/ for reminder).
