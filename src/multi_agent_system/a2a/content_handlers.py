...
class ContentHandlerRegistry:
    ...
    def get_handler_for_part(self, part: A2APart) -> Optional[ContentHandler]:
        """Get handler for part."""
        return self.get_handler(part.content_type)
    ...
    def serialize_part_content(self, part: A2APart) -> str:
        """Serialize part content using appropriate handler."""
    ...
    def deserialize_part_content(self, part: A2APart) -> Any:
        """Deserialize part content using appropriate handler."""
        handler = self.get_handler_for_part(part)
        if handler:
            return handler.deserialize(str(part.content))
        else:
            # Default to string deserialization
            return str(part.content)
    
    def validate_part_content(self, part: A2APart) -> List[str]:
        """Validate part content using appropriate handler."""
        handler = self.get_handler_for_part(part)
        if handler:
            return handler.validate(part.content)
        else:
            return ["No handler found for content type: " + part.content_type]
