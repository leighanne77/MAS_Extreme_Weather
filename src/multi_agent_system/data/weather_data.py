...
class NOAAWeatherData:
    ...
    def _save_to_cache(self, data: dict | bytes, cache_path: Path):
        """Save data to cache."""
        try:
    ...
    def _load_from_cache(self, cache_path: Path, format: str) -> dict | pd.DataFrame | bytes:
    ...
    def clear_cache(self, older_than: timedelta | None = None):
        """
        Clear the cache.
