...
class DataManager:
    ...
    async def fetch_data(self, source_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.circuit_breaker.is_allowed(source_name):
    ...
    async def fetch_risk_data(self, risk_type: RiskType, location: str) -> Dict[str, Any]:
        ...
        try:
            # Check circuit breaker
            if not self.circuit_breaker.is_allowed(f"risk_data_{risk_type.value}"):
                raise Exception(f"Circuit breaker is open for {risk_type.value}")
            ...
            # Record failure in circuit breaker
            self.circuit_breaker.record_failure(f"risk_data_{risk_type.value}")
