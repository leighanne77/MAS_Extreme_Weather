...
class ExplicitInvocation:
    ...
    async def _get_cached_result(self, session: AnalysisSession) -> dict[str, Any] | None:
    ...
    async def _cache_result(self, session: AnalysisSession, result: dict[str, Any]) -> None:
        ...
        try:
            ...
            session.context[cache_key] = cache_data
        except Exception as e:
    ...
    async def _summarize_response(self, result: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result.get("result"), dict):
                ...
                summary["keys"] = list(result_data.keys())
                summary["has_data"] = bool(result_data)
                ...
                if "confidence_level" in result_data:
                    summary["confidence"] = result_data["confidence_level"]

            result["summary"] = summary
            return result

        except Exception as e:
            logger.warning(f"Error summarizing response: {e}")
            return result
